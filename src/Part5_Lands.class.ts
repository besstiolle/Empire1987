...
export class Lands extends Party {
  ...
  protected static nbAttacks:number = 0
    ...
    static choiceQttOst(opponent:number){
      //console.info("choiceQttOst(" + opponent + ")");
      if(Lands.nbAttacks > UserUtils.getNbAttacksMax(Game.getInstance().getCurrentUser())){
        Game.getInstance().addError(Errors.notEnoughtNobles());
        return Lands.choosingOpponent();
      }

      let opponents = Game.getInstance().getOpponentsAsArray();
      ...
      //opponent = myself
      if(Game.getInstance().getCurrentUser().id == opponents[opponent - 1].getId()){
        //console.info("pas soi même");
        Game.getInstance().addError(Errors.fightMyself());
        return Lands.choosingOpponent();
      }
      KB.listenTyping([
    ...
    //Selection du nombre de soldat
    static doFight(key:number, defenderUserId:number){
      let ost:number = parseInt(KB.buffer())
      if(isNaN(ost) || ost === 0){
        return Lands.choosingOpponent();
      }
      ...
      if(Game.getInstance().getCurrentUser().ost < ost){
        Game.getInstance().addError(Errors.notEnoughtOst());
        return Lands.choosingOpponent();
      }
      ...
      if(defenderUserId == 0){
        promiseCombat = Combat.execute(Game.getInstance().barbares, Game.getInstance().getCurrentUser(), ost);
      } else {
        promiseCombat = Combat.execute(Game.getInstance().getUserById(defenderUserId), Game.getInstance().getCurrentUser(), ost);
      }
